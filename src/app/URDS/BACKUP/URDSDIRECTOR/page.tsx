...
const handleDelete = async (e: React.MouseEvent, id: string) => {
  ...
  try {
    // 🔥 CALL API DELETE
    await apiFetch(
      `http://192.168.254.142:5000/api/announcements/${id}`,
      {
        method: "DELETE",
      }
    );
...
const mappedUpdated = {
  id: updated.announcement_id,
  title: updated.proposal_title,
  description: updated.description,
  startDate: updated.start_date,
  deadline: updated.deadline,
  guidelines: updated.guidelines_attachment,
  status: updated.status,
  createdAt: updated.timestamp,
};

// ✅ UPDATE STATE WITH MAPPED OBJECT
setAnnouncements(prev =>
  prev.map(ann =>
    ann.id === editingId ? mappedUpdated : ann
  )
);
...
// 🔥 MAP THE ID CORRECTLY
const mappedCreated = {
  id: created.announcement_id, // ✅ FIX
  title: created.proposal_title,
  description: created.description,
  startDate: created.start_date,
  deadline: created.deadline,
  guidelines: created.guidelines_attachment,
  status: created.status,
  createdAt: created.timestamp,
  
};

setAnnouncements(prev => [mappedCreated, ...prev]);
    }
