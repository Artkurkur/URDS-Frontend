...
import { X, Download, Mail, Building2, Paperclip } from 'lucide-react';

interface ResearchGuidelinesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function ResearchGuidelinesModal({ isOpen, onClose }: ResearchGuidelinesModalProps) {
  
  if (!isOpen) return null;

  const handleDownloadPDF = () => {
    // 1. Define the content (This was causing the error because it wasn't being used below)
    const content = `
...
Office: University Research and Development Services Office
    ...
    // 2. Use the content to create a downloadable file (This fixes the error)
  ...
  return (
    <div 
      className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black bg-opacity-60 backdrop-blur-sm transition-opacity"
      onClick={onClose}
    >
      <div 
        className="relative bg-white rounded-3xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b-2 border-gray-200 shrink-0">
        ...
        {/* Scrollable Body */}
        <div className="overflow-y-auto p-8 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
          {/* Introduction */}
