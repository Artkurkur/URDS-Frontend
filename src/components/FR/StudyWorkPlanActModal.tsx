...
interface StudyState {
  ...
  activeTab?: 'proposal' | 'workplan' | 'budget';
  [key: string]: string | number | boolean | undefined | null; 
}

interface StudyWorkPlanActModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveDraft: () => void;
  onSubmit: () => void;
...
export default function StudyWorkPlanActModal({ 
  isOpen, 
  onClose, 
  onSaveDraft, 
  onSubmit, 
  newStudy, 
  setNewStudy 
}: StudyWorkPlanActModalProps) {
  const [activeTab, setActiveTab] = useState<'proposal' | 'workplan' | 'budget'>('workplan');

  // Update newStudy.activeTab whenever activeTab changes
  useEffect(() => {
    setNewStudy((prev) => ({ ...prev, activeTab }));
  }, [activeTab, setNewStudy]);
    ...
    const renderInfoRow = (label: string, field: string) => (
      <tr>
        ...
        <td colSpan={4} className="p-0 border border-black bg-white">
          <input 
            type="text" 
            className="w-full h-full p-2 outline-none text-black font-bold"
            value={String(newStudy[field] || "")}
            onChange={(e) => handleChange(field, e.target.value)}
          />
        </td>
